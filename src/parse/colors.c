...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/08/02 17:54:19 by maragao           #+#    #+#             */
/*   Updated: 2023/08/02 17:54:21 by maragao          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	get_colors(t_file *file)
{
	...
	{
		if (ft_strnstr(file->file[i], "F ",
				ft_strlen(file->file[i])) != NULL)
		{
			if (file->colors[0] != NULL)
				return (1);
			file->colors[0] = ft_strdup(file->file[i]);
		}
		else if (ft_strnstr(file->file[i], "C ",
				ft_strlen(file->file[i])) != NULL)
		{
